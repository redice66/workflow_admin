...
import { Plus, GripVertical, Trash2, Eye, Settings } from "lucide-react"
import { useToast } from "@/components/ui/use-toast"
import { type Project, type ProjectField, getProjectFields } from "@/lib/api/projects-api"
import { DragDropContext, Droppable, Draggable } from "@hello-pangea/dnd"
import { Field } from "@/lib/fields-store"
import { getAllFields } from "@/lib/api/fields-api"

interface ProjectFieldsTabProps {
  project: Project
}
...
export function ProjectFieldsTab({ project }: ProjectFieldsTabProps) {
  const { toast } = useToast()
  const [availableFields, setAvailableFields] = useState<Field[]>([])
  const [loading, setLoading] = useState(false)

  const [fields, setFields] = useState<ProjectFieldConfig[]>([
    {
      id: "1",
      fieldId: "field-1",
      fieldName: "title",
  ...
  const addNewField = () => {
    const newField: ProjectFieldConfig = {
  ...
  const updateField = (fieldId: string, updates: Partial<ProjectFieldConfig>) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground bg-muted/50 p-4 rounded-lg">
        <p>• 可以通过拖拽左侧的图标来调整字段排序</p>
        <p>• 字段支持拖拽排序，表头包含字段名称、展示名、元件类型、是否必填、是否可编辑、操作</p>
      </div>
